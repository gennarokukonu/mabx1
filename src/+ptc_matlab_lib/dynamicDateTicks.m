...
%
% Revision History
%   0.01.[BA]2010.04.12 Changed formats: Sep/10 -> Sep-2010, 09/25(/10) ->
%     25-Sep(-2010), 09/25(/10)-18:00(:00) -> 25-Sep(-2010) 18:00(:00).
%   0.00.[AD]2010.03.24 Release by Ameya Deoras on FEX.
...
for i = 1:length(axH)
    ...
    updateDateLabel('', struct('Axes', axH(i))); % Call once to ensure proper formatting
end
...
% ------------ End of dynamicDateTicks-----------------------
    ...
    function updateDateLabel(obj, ev)
        ...
        if regexpi(labels(1,:), '([a-z]{3})|(\d\d/\d\d)', 'once') % Tick format is either mmm or mm/dd
            ...
            newlabels(ind) = cellstr(datestr(ticks(ind), '-yyyy')); %BA changed year format
            %======== [BA] Reset tick labels to different format ==========
            if regexpi(labels(1,:),'(\d\d/\d\d)','once')
               labels = cellstr(datestr(ticks,'dd-mmm'));
            end
            %==============================================================
        ...
        elseif any(labels(1,:) == ':') % Tick format is HH:MM
            ...
            ind = find(diff(da))+1; 
            newlabels{1}   = datestr(ticks(1), 'dd-mmm-yy!'); % Add month/day/year to first tick [BA] changed format from 'mm/dd/yy'
            newlabels(ind) = cellstr(datestr(ticks(ind), 'dd-mmm!')); % Add month/day to ticks where day changes [BA] changed format from 'mm/dd'
            newlabels = regexprep(newlabels,'!',' ');
